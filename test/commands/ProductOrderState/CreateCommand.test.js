...
import CreateCommand from '../../../src/commands/ProductOrderState/CreateCommand.js';
...
test('CreateCommand should create a new product order state', async () => {
    await commandService.invoke(new CreateCommand('Available2'));
    const state = await testDB.db.ProductOrderState.findOne({ 
...
test('CreateCommand should throw an error if a product order state if name already exist', async () => {
