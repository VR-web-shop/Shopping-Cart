...
import DeleteCommand from '../../../src/commands/Product/DeleteCommand.js';
...
test('DeleteCommand should soft delete a product', async () => {
    const client_side_uuid = testDB.data.products[0].client_side_uuid;
    ...
    const removed = await testDB.db.ProductRemoved.findOne({ where: { 
    ...
    expect(removed.product_client_side_uuid).toBe(client_side_uuid);
