...
import ReadCollectionQuery from '../../../src/queries/CartState/ReadCollectionQuery.js';
...
test('ReadCollectionQuery should read all cart states', async () => {
  const { rows, count } = await queryService.invoke(new ReadCollectionQuery());

  expect(rows).toHaveLength(2);
  expect(rows[0].name).toBe('Inactive');
  expect(rows[1].name).toBe('Active');
  expect(count).toBe(2);
});

test('ReadCollectionQuery should be able to paginate cart states if page and limit is provided', async () => {
